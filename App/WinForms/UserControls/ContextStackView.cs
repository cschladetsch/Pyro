    ...
    public partial class ContextStackView
        ...
        private void OnIpChanged(Continuation continuation, int last, int current) {
            if (last >= codeView.Items.Count) {
                foreach (ListViewItem item in codeView.Items) item.BackColor = Color.White;
            }
            else {
        ...
        private void UpdateContinuation(Continuation current) {
            ...
            var n = 0;
            foreach (var item in current.Code) AddCodeItem(n++, item);

            scopeView.Items.Clear();
            foreach (var item in current.Scope) AddScopeItem(item.Key, item.Value);
        ...
        private void UpdateContextStack(Executor executor, List<Continuation> contexts) {
            ...
            var n = 0;
            foreach (var cont in contexts) AddContinuationToStack(n++, cont);
        }
        ...
        private void UpdateScope(Continuation continuation) {
            scopeView.Items.Clear();
            foreach (var scoped in continuation.Scope) AddScopeItem(scoped.Key, scoped.Value);
        }

        private void UpdateCode(Continuation continuation) {
            codeView.Items.Clear();
            var n = 0;
            foreach (var op in continuation.Code) AddCodeItem(n++, op);
        }
        ...
        private void stepNextClicked(object sender, EventArgs e) {
            ...
            try {
                ...
                if (current != null) {
                    MessageBox.Show("Nothing to Continue", "If a tree falls in the woods, does anyone hear it?"
                        , MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                MessageBox.Show(ex.Message, $"Error executing {current.Code[current.Ip]}", MessageBoxButtons.OK
